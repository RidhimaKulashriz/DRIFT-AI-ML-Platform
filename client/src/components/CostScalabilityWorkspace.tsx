        ...
        <>
          <section className="cost-control-panel" aria-label="Cost model controls">
            ...
            <div className="cost-input-grid">
              <label>BENCHMARK SCENARIO<select id="costscalabilityworkspace-field-1" name="costscalabilityworkspace-field-1" value={scenarioId} onChange={event => setScenarioId(event.target.value)}>{COST_SCENARIOS.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
              <label>VERIFIED DISTANCE · KM<input id="costscalabilityworkspace-field-2" name="costscalabilityworkspace-field-2" type="number" min={1} step={1} value={distanceKm} onChange={event => setDistanceKm(safeNumber(event.target.value, distanceKm, 1))} /></label>
              <label>RAIL SENSOR NODES<input id="costscalabilityworkspace-field-3" name="costscalabilityworkspace-field-3" type="number" min={0} step={1} value={railNodes} disabled={!isRupeeScenario} onChange={event => setRailNodes(safeNumber(event.target.value, railNodes))} /><small>{isRupeeScenario ? `${formatINR(RAIL_SENSOR_NODE_COST)} per configurable prototype node` : "Available on INR scenarios only; road study is reported in soles."}</small></label>
            </div>
          </section>
        ...
        <>
          <section className="cost-control-panel" aria-label="Scalability model controls">
            ...
            <div className="cost-input-grid scale-input-grid">
              <label>FLEET SIZE<select id="costscalabilityworkspace-field-4" name="costscalabilityworkspace-field-4" value={fleetSize} onChange={event => setFleetSize(Number(event.target.value))}>{FLEET_OPTIONS.map(value => <option key={value} value={value}>{value.toLocaleString("en-IN")} drone{value === 1 ? "" : "s"}</option>)}</select></label>
              <label>KM PER MISSION<input id="costscalabilityworkspace-field-5" name="costscalabilityworkspace-field-5" type="number" min={0.5} step={0.5} value={kmPerMission} onChange={event => setKmPerMission(safeNumber(event.target.value, kmPerMission, 0.5))} /></label>
              <label>MISSIONS / DAY / DRONE<input id="costscalabilityworkspace-field-6" name="costscalabilityworkspace-field-6" type="number" min={1} step={1} value={missionsPerDay} onChange={event => setMissionsPerDay(safeNumber(event.target.value, missionsPerDay, 1))} /></label>
              <label>OPERATING DAYS / YEAR<input id="costscalabilityworkspace-field-7" name="costscalabilityworkspace-field-7" type="number" min={1} max={365} step={1} value={operatingDays} onChange={event => setOperatingDays(Math.min(365, safeNumber(event.target.value, operatingDays, 1)))} /></label>
              <label>UTILISATION<input id="costscalabilityworkspace-field-8" name="costscalabilityworkspace-field-8" type="number" min={10} max={100} step={5} value={utilisation} onChange={event => setUtilisation(Math.min(100, safeNumber(event.target.value, utilisation, 10)))} /><small>100% is the supplied theoretical ceiling.</small></label>
              <label>TARGET NETWORK · KM<input id="costscalabilityworkspace-field-9" name="costscalabilityworkspace-field-9" type="number" min={1} step={50} value={targetNetworkKm} onChange={event => setTargetNetworkKm(safeNumber(event.target.value, targetNetworkKm, 1))} /></label>
              <label>PRIORITY ROAD SHARE · %<input id="costscalabilityworkspace-field-10" name="costscalabilityworkspace-field-10" type="number" min={1} max={100} step={5} value={priorityShare} onChange={event => setPriorityShare(Math.min(100, safeNumber(event.target.value, priorityShare, 1)))} /><small>Workbook illustration defaults to 30%.</small></label>
              <label>RAIL SENSOR NODES<input id="costscalabilityworkspace-field-11" name="costscalabilityworkspace-field-11" type="number" min={0} step={1} value={railNodes} onChange={event => setRailNodes(safeNumber(event.target.value, railNodes))} /><small>{formatINR(RAIL_SENSOR_NODE_COST)} per configurable node.</small></label>
            </div>
          </section>
