...
export default function MissionApplicationsWorkspace({ modeKey }: { modeKey: MissionMode }) {
  ...
  return <section className="workspace-page mission-app-workspace" style={{ "--mission-accent": mode.accent } as CSSProperties}>
    ...
    <div className="mission-app-grid"><article className="mission-map-card"><div className="mission-card-heading"><div><span className="eyebrow">SPATIOTEMPORAL GRAPH · {graph.nodes.length} NODES / {graph.edges.length} EDGES</span><h3>{mode.outcome} topology · {result.distance.toFixed(3)} km</h3></div><button type="button" className="secondary-action" onClick={runAnalysis}><Play /> RUN MODEL</button></div><div className="mission-map mission-graph-canvas"><div className="mission-grid-lines" />{graph.edges.map((edge, index) => { const from = graph.nodes.find(node => node.id === edge.from)!; const to = graph.nodes.find(node => node.id === edge.to)!; return <div key={`${edge.from}-${edge.to}-${index}`} className="graph-edge" title={`${edge.relation} · ${edge.weight}%`} style={{ left: `${from.x}%`, top: `${from.y}%`, width: `${Math.hypot(to.x - from.x, to.y - from.y)}%`, transform: `rotate(${Math.atan2(to.y - from.y, to.x - from.x) * 180 / Math.PI}deg)`, opacity: 0.25 + edge.weight / 150 }}><span>{edge.relation}</span></div>; })}{graph.nodes.map(node => <button key={node.id} type="button" className={`graph-node graph-node-${node.kind}`} style={{ left: `${node.x}%`, top: `${node.y}%`, "--node-value": node.value } as React.CSSProperties} onClick={() => node.pointId ? togglePointStatus(node.pointId) : toast.info(`${node.label} · ${node.value}% state confidence`)} aria-label={`${node.label}, ${node.kind}, ${node.value}% confidence`}><span className="graph-node-core">{node.pointId ?? "·"}</span><small>{node.label}</small><em>{node.value}%</em></button>)}<div className="mission-map-label label-top"><MapPinned />{mode.outcome.toUpperCase()} · LIVE TOPOLOGY</div><div className="mission-map-label label-bottom">{activeLayer + 1} · {mode.layers[activeLayer]} · click observation node to update state</div></div><div className="mission-layer-row">{mode.layers.map((layer, index) => <button key={layer} className={activeLayer === index ? "active" : ""} onClick={() => setActiveLayer(index)}><Layers />{layer}</button>)}</div></article>
