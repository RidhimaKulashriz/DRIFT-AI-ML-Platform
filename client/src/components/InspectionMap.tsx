...
export function InspectionMap({ defects, telemetry, selectedId, streetViewRequest = 0, onSelect, className }: InspectionMapProps) {
  ...
  useEffect(() => {
    ...
    projectMarkers.current = []; contextMarkers.current = [];
    ...
    validDefects.forEach(({ defect, point }, index) => {
      ...
      const markerLabel = isTransient ? String(index + 1) : defect.severity[0]!.toUpperCase();
      const marker = new window.google.maps.Marker({ map, position: point, title: `${isTransient ? "SIMULATED DEMO advisory" : "DRIFT project finding"}: ${defect.label}`, zIndex: selected ? 10_000 : 1_000 + index, label: { text: markerLabel, color: "#ffffff", fontWeight: "700", fontSize: isTransient ? "11px" : "12px" }, icon: { path: window.google.maps.SymbolPath.CIRCLE, fillColor: colors[defect.severity], fillOpacity: 1, strokeColor: "#ffffff", strokeWeight: selected ? 4 : 2, scale: selected ? 14 : 12 } });
      marker.addListener("click", () => {
        onSelect(defect.id); setStreetViewStatus("idle");
      ...
      projectMarkers.current.push(marker); bounds.extend(point);
    });
    validTelemetry.forEach(point => {
      const marker = new window.google.maps.Marker({ map, position: point, clickable: false, zIndex: 50, icon: { path: window.google.maps.SymbolPath.CIRCLE, fillColor: "#16b7d4", fillOpacity: .5, strokeColor: "#ffffff", strokeWeight: .75, scale: 2 } });
      projectMarkers.current.push(marker); bounds.extend(point);
    });
    publicNbiBridgeContext.forEach(context => {
      ...
      marker.addListener("click", () => { infoWindow.setContent(`<div style="max-width:250px;font:13px Arial,sans-serif"><strong>Public NBI context only</strong><br/>${context.title} · Structure ${context.structureNumber}<br/>Published deck-condition field: ${context.deckCondition}<br/><em>2025 public inventory record. Not a DRIFT site, live defect, ticket, or safety determination.</em></div>`); infoWindow.open({ map, anchor: marker }); });
      contextMarkers.current.push(marker);
    });
    if (validDefects.length || validTelemetry.length) map.fitBounds(bounds, 104);
