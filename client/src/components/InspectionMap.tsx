...
export function InspectionMap({ defects, telemetry, selectedId, streetViewRequest = 0, onSelect, className }: InspectionMapProps) {
  ...
  useEffect(() => {
    ...
    if (!map.getPane("driftTiles")) {
      ...
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", { maxZoom: 19, attribution: "© OpenStreetMap contributors", pane: "driftTiles" }).addTo(map);
    }
    ...
    publicNbiBridgeContext.forEach(context => L.circleMarker([context.latitude, context.longitude], { radius: 6, color: "#ede9fe", weight: 2, fillColor: "#7c3aed", fillOpacity: 0.85 }).bindTooltip(`NBI context · ${context.title}`, { direction: "top" }).addTo(layer));
    const boundsPoints = [...points, ...(shouldShowTelemetry ? validTelemetry : []), ...publicNbiBridgeContext.map(context => ({ lat: context.latitude, lng: context.longitude }))];
