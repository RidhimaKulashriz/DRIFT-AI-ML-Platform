...
export function InspectionMap({ defects, telemetry, selectedId, imageryRequest = 0, onSelect, className }: InspectionMapProps) {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => map.invalidateSize(), 0);
  }, [onSelect, selectedDefect, selectedId, selectedImageryOffset, selectedKartaViewPhoto, showTelemetry, validDefects, validTelemetry]);
  ...
  return <section className={cn("relative min-h-[500px] overflow-hidden border border-slate-700 bg-slate-950", className)} aria-label="DRIFT real geographic inspection map">
    {selectedKartaViewPhoto && <div className="absolute inset-y-0 left-0 z-0 w-[46%] overflow-hidden border-r-2 border-emerald-300/80 bg-slate-900"><img className="h-full w-full object-cover" src={selectedKartaViewPhoto.imageUrl} alt="Real KartaView street-level view near the selected finding" /><div className="absolute inset-x-0 bottom-0 bg-slate-950/90 p-3 text-[10px] font-bold uppercase tracking-[.12em] text-white">REAL STREET VIEW · KARTAVIEW<br /><span className="font-normal normal-case tracking-normal text-amber-200">Public street context only · {selectedImageryOffset ?? 0} m from advisory</span></div></div>}
    <div ref={mapElement} className={cn("absolute inset-y-0 right-0", selectedKartaViewPhoto ? "w-[54%]" : "inset-x-0")} />
