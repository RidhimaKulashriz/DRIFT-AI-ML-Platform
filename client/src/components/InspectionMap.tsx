...
export function InspectionMap({ defects, telemetry, selectedId, imageryRequest = 0, onSelect, className }: InspectionMapProps) {
  ...
  const googleStreetViewUrl = useMemo(() => {
    ...
    url.searchParams.set("viewpoint", `${kartaViewCenter.lat},${kartaViewCenter.lng}`);
    return url.toString();
  }, [kartaViewCenter.lat, kartaViewCenter.lng]);
  const googlePinnedMapUrl = useMemo(() => {
    const url = new URL("https://www.google.com/maps/search/");
    url.searchParams.set("api", "1");
    url.searchParams.set("query", `${kartaViewCenter.lat},${kartaViewCenter.lng}`);
    return url.toString();
  }, [kartaViewCenter.lat, kartaViewCenter.lng]);
    ...
    try {
      ...
      for (const { tileX, tileY } of tileRequests) {
        ...
        try {
          ...
          const response = await fetch(tileUrl, { headers: { Accept: "application/x-protobuf" }, signal: controller.signal });
  ...
  const moveKartaViewPhoto = (delta: number) => {
    if (!selectedKartaViewPhoto || !kartaViewPhotos.length) return;
    const currentIndex = Math.max(0, kartaViewPhotos.findIndex(photo => photo.id === selectedKartaViewPhoto.id));
    const nextIndex = Math.min(kartaViewPhotos.length - 1, Math.max(0, currentIndex + delta));
    setSelectedKartaViewPhoto(kartaViewPhotos[nextIndex] ?? selectedKartaViewPhoto);
  };
