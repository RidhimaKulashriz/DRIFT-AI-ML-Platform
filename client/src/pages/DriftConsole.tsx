...
export default function DriftConsole() {
  ...
  const acceptContractorTicket = trpc.drift.accountability.tickets.accept.useMutation({
    onSuccess: () => { toast.success("Ticket acceptance recorded in the audit trail."); utils.drift.accountability.overview.invalidate(); },
    onError: error => toast.error(error.message),
  });
  const startContractorTicket = trpc.drift.accountability.tickets.start.useMutation({
    onSuccess: () => { toast.success("Ticket moved to in progress."); utils.drift.accountability.overview.invalidate(); },
    onError: error => toast.error(error.message),
  });
  const addContractorTicketNote = trpc.drift.accountability.tickets.addNote.useMutation({
    onSuccess: () => { toast.success("Contractor note recorded in the audit trail."); setContractorNote(""); utils.drift.accountability.overview.invalidate(); },
    onError: error => toast.error(error.message),
  });
  ...
  const assignedContractorTickets = (accountability.data?.tickets ?? []).filter(ticket => ticket.assignedUserId === user?.id);
  ...
  const buildTransientBriefing = () => {
    ...
    const entries = transientSimulatorRun.findings.map((finding, index) => `- Demo candidate ${index + 1}: ${finding.title} · advisory class ${finding.label} · model confidence ${Math.round(finding.confidence * 100)}% · simulated severity ${finding.score.severity.toUpperCase()} · temporary coordinates ${finding.latitude.toFixed(6)}, ${finding.longitude.toFixed(6)}`).join("\n");
    ...
    <div className="drift-shell">
      ...
      <main className="main-stage">
        ...
        {workspace === "evidence" && <section className="workspace-page evidence-workspace">
          <div className="workspace-header"><div><span className="eyebrow">SECURE MISSION MEDIA</span><h2>Evidence vault</h2></div><div><input ref={filePickerRef} className="file-picker" type="file" accept="image/*,video/*" onChange={event => handleEvidenceFile(event.target.files?.[0])} /><button type="button" className="primary-action" onClick={() => filePickerRef.current?.click()} disabled={!canOperate || !persistenceAvailable || uploadEvidence.isPending} title={!canOperate ? "Sign in as an engineer or administrator to upload original drone media." : !persistenceAvailable ? persistenceMessage : undefined}><Upload /> {uploadEvidence.isPending ? "STORING" : !canOperate ? "SIGN IN TO UPLOAD" : !persistenceAvailable ? "PERSISTENCE REQUIRED" : "UPLOAD EVIDENCE"}</button></div></div>
        ...
        {workspace === "reports" && <section className="workspace-page reports-workspace">
          ...
          {!persistenceAvailable && <article className="report-preview-panel"><div><span className="eyebrow">BROWSER-ONLY WALKTHROUGH · NO PERSISTENCE</span><h3>Transient simulator briefing</h3><p>Generate and download a temporary Markdown walkthrough from the currently visible simulator data. It is not an engineering report, evidence file, inspection record, or contractor instruction.</p></div><button type="button" className="secondary-action" onClick={buildTransientBriefing} disabled={!transientSimulatorRun}> {transientSimulatorRun ? "BUILD TRANSIENT BRIEFING" : "RUN TRANSIENT DEMO FIRST"} </button>{transientBriefing && <><pre className="ai-brief">{transientBriefing}</pre><button type="button" className="primary-action" onClick={downloadTransientBriefing}>DOWNLOAD TRANSIENT BRIEFING</button></>}</article>}
