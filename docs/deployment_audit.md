...
| Backend/auth | `JWT_SECRET`, optional external OAuth provider values, `DRIFT_EXTERNAL_OAUTH_ENABLED=true`, owner identity, Forge URL/key | No for public monitoring | Configure a supported external OAuth provider and set the explicit enablement flag before enabling protected engineering operations | No safe anonymous access to protected operations; the backend returns an actionable `503` for sign-in attempts while public read procedures remain available |
