...
import { runVisionDetections, runVisionInference } from "../services/mlInference";
  ...
  app.post("/api/drift/evidence", async (req, res) => {
    ...
    try {
      ...
      if (body.runInference === true && body.mediaKind === "photo" && typeof body.assetId === "number" && typeof body.assetCriticality === "number" && typeof body.latitude === "number" && typeof body.longitude === "number") {
        const inferences = body.liveFrame === true
          ? await runVisionDetections({ fileName: String(body.fileName), imageBase64: base64Payload, latitude: body.latitude, longitude: body.longitude, assetCriticality: body.assetCriticality, priorOpenDefects: typeof body.priorOpenDefects === "number" ? body.priorOpenDefects : 0, inspectionDomain: typeof body.inspectionDomain === "string" ? body.inspectionDomain : undefined, captureZone: typeof body.captureZone === "string" ? body.captureZone : undefined, productionOnly: true })
          : [await runVisionInference({ fileName: String(body.fileName), imageBase64: base64Payload, latitude: body.latitude, longitude: body.longitude, assetCriticality: body.assetCriticality, priorOpenDefects: typeof body.priorOpenDefects === "number" ? body.priorOpenDefects : 0, inspectionDomain: typeof body.inspectionDomain === "string" ? body.inspectionDomain : undefined, captureZone: typeof body.captureZone === "string" ? body.captureZone : undefined, productionOnly: false })].filter((value): value is NonNullable<typeof value> => Boolean(value));
        if (!inferences.length) {
          ...
          return res.status(201).json({ ...result, inferences: [], qualityGate: { status: "review", action: "production-inference-unavailable" } });
        }
        const assetId = Number(body.assetId);
        const latitude = Number(body.latitude);
        const longitude = Number(body.longitude);
        const persisted = await Promise.all(inferences.map(inference => persistInferenceDefect({ missionId: Number(body.missionId), assetId, evidenceId: result.id, latitude, longitude, inference, inspectionDomain: typeof body.inspectionDomain === "string" ? body.inspectionDomain : undefined, correlationKey: typeof body.correlationKey === "string" ? `${String(body.correlationKey)}:${inference.label}` : undefined, createdBy: undefined })));
        const detections = persisted.map((defect, index) => ({ defectId: defect.defectId, label: inferences[index]?.label, confidence: inferences[index]?.confidence, severity: inferences[index]?.score.severity, boundingBox: inferences[index]?.boundingBox }));
        const response = { ...result, inferences: persisted, qualityGate: { status: "review", action: "engineer-review-required" } };
