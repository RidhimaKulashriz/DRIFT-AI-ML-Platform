...
export const appRouter = router({
  ...
  drift: router({
    ...
    reconstruction: router({
      list: publicProcedure.query(async () => { const db = await getDb(); if (!db) return []; return db.select().from(reconstructionJobs).orderBy(desc(reconstructionJobs.createdAt)).limit(25); }),
