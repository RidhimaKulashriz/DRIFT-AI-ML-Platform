...
import { z } from "zod";
import { acknowledgeAlert, addReview, addTelemetryRecord, approveKnowledgeDocumentRecord, approveRoutingDecisionRecord, closeContractorTicketRecord, createAssetRecord, createAuthorityRecord, createContractorRecord, createContractorTicketRecord, createDemoMissionRecord, createEvidenceRecord, createHardwareCaptureMission, createKnowledgeDocumentRecord, createRoutingRuleRecord, createSlaRuleRecord, deleteAssetRecord, generateMissionReport, getAccountabilityOverview, getMapData, getMissionOverview, listAlerts, listAssets, listAuditEvents, listCorrelatedDefects, listDemoEvidence, listFilteredDefects, listKnowledgeRetrievalRuns, listMissionEvidence, listPublishedPublicStatuses, listReportRecords, persistInferenceDefect, prepareHandoffPackageRecord, publishPublicStatusRecord, resolveTicketRoutingRecord, retrieveApprovedKnowledge, updateAssetRecord, verifyContractorTicketRecord } from "./db";
...
export const appRouter = router({
  ...
  drift: router({
    ...
    reports: router({
      ...
      generate: publicProcedure.input(z.object({ missionId: z.number().int().positive() })).mutation(({ input }) => generateMissionReport({ missionId: input.missionId })),
    }),
    accountability: router({
      overview: publicProcedure.query(() => getAccountabilityOverview()),
      publicStatuses: publicProcedure.query(() => listPublishedPublicStatuses()),
      knowledge: router({
        registerDraft: protectedProcedure.input(z.object({ projectScope: z.string().trim().min(2).max(160), title: z.string().trim().min(3).max(300), documentType: z.string().trim().min(2).max(120), version: z.string().trim().min(1).max(80), permittedRoles: z.array(z.enum(["admin", "engineer", "user", "citizen"])).min(1).max(4), sourceReference: z.string().url().max(768).optional(), content: z.string().trim().min(80).max(100000) })).mutation(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin"]); return createKnowledgeDocumentRecord({ ...input, createdBy: ctx.user.id }); }),
