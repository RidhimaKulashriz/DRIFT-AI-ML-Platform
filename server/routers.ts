...
export const appRouter = router({
  ...
  drift: router({
    ...
    reconstruction: router({
      ...
      uploadSource: publicProcedure.input(z.object({ fileName: z.string().min(1).max(255), mimeType: z.string().min(3).max(120), base64: z.string().min(32).max(350_000_000) })).mutation(async ({ input }) => { const encoded = input.base64.replace(/^data:[^;]+;base64,/, ""); const bytes = Buffer.from(encoded, "base64"); if (!bytes.length || bytes.length > 250 * 1024 * 1024) throw new Error("Video upload must be between 1 byte and 250 MB."); const stored = await storagePut(`drift/reconstruction/source/${Date.now()}-${input.fileName.replace(/[^a-zA-Z0-9._-]/g, "_")}`, bytes, input.mimeType); const sourceUrl = stored.url.startsWith("http") ? stored.url : await storageGetSignedUrl(stored.key); if (!sourceUrl.startsWith("https://")) throw new Error("Uploaded source storage did not return an HTTPS URL reachable by the reconstruction worker."); return { sourceUrl, storageKey: stored.key, fileName: input.fileName, mimeType: input.mimeType, sizeBytes: bytes.length }; }),
