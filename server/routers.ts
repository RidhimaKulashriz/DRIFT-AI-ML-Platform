...
import { storageGetSignedUrl, storagePut, storagePutWithFallback } from "./storage";
...
import { reconstructionJobs, reconstructionSourceUploads, assets, cameraSources, cctvCandidates } from "../drizzle/schema";
...
export const appRouter = router({
  ...
  drift: router({
    ...
    reconstruction: router({
      ...
      uploadSource: publicProcedure.input(z.object({ fileName: z.string().min(1).max(255), mimeType: z.string().min(3).max(120), base64: z.string().min(32).max(350_000_000) })).mutation(async ({ ctx, input }) => { const encoded = input.base64.replace(/^data:[^;]+;base64,/, ""); const bytes = Buffer.from(encoded, "base64"); if (!bytes.length || bytes.length > 250 * 1024 * 1024) throw new Error("Video upload must be between 1 byte and 250 MB."); const stored = await storagePutWithFallback(`drift/reconstruction/source/${Date.now()}-${input.fileName.replace(/[^a-zA-Z0-9._-]/g, "_")}`, bytes, input.mimeType); if (stored.attachmentData) { const db = await getDb(); if (!db) throw new Error("Database is required for the storage fallback."); await db.insert(reconstructionSourceUploads).values({ storageKey: stored.key, fileName: input.fileName, mimeType: input.mimeType, attachmentData: stored.attachmentData }); } const forwardedProto = String(ctx.req.header("x-forwarded-proto") ?? ctx.req.protocol).split(",")[0].trim(); const origin = `${forwardedProto}://${ctx.req.get("host")}`; const sourceUrl = stored.url.startsWith("http") ? stored.url : new URL(stored.url, origin).toString(); if (!sourceUrl.startsWith("https://")) throw new Error("Uploaded source storage did not return an HTTPS URL reachable by the reconstruction worker."); return { sourceUrl, storageKey: stored.key, fileName: input.fileName, mimeType: input.mimeType, sizeBytes: bytes.length }; }),
