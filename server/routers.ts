...
import { z } from "zod";
import { acknowledgeAlert, acceptContractorTicketRecord, addContractorTicketNoteRecord, addReview, addTelemetryRecord, analyzeOperationalWorldGraph, approveKnowledgeDocumentRecord, approveRoutingDecisionRecord, assignContractorUserRecord, closeContractorTicketRecord, createAssetRecord, createAuthorityRecord, createCameraSourceRecord, createContractorRecord, createContractorTicketRecord, createCctvCandidateRecord, createDemoMissionRecord, createEvidenceRecord, createHardwareCaptureMission, createKnowledgeDocumentRecord, createRoutingRuleRecord, createSlaRuleRecord, deleteAssetRecord, generateMissionReport, getAccountabilityOverview, getFinalIntelligence, getMapData, getMissionOverview, getOperationalIntelligence, getOperationalWorldStateStatus, getPublicAccountabilityOverview, getPublicMissionOverview, getReadOnlySchemaReadiness, listAlerts, listAssets, listAssignedContractorTickets, listAuditEvents, listCorrelatedDefects, listDemoEvidence, listFilteredDefects, listKnowledgeRetrievalRuns, listMissionEvidence, listPublishedPublicStatuses, listReportRecords, persistInferenceDefect, prepareHandoffPackageRecord, prepareUavFollowUpRecommendationRecord, publishPublicStatusRecord, queryOperationalWorldGraph, registerAuthorizedSecurityObservationRecord, replayOperationalEvents, resolveTicketRoutingRecord, retrieveApprovedKnowledge, reviewCctvCandidateRecord, simulateOperationalTwinFailure, startContractorTicketRecord, updateAssetRecord, updateEvidenceDetections, verifyContractorTicketRecord } from "./db";
...
import { storagePut, storagePutWithFallback } from "./storage";
...
export const appRouter = router({
  ...
  drift: router({
    ...
    intelligence: router({
      snapshot: protectedProcedure.query(({ ctx }) => { requireDriftRole(ctx.user, ["admin", "engineer", "user"]); return getOperationalIntelligence(); }),
      worldState: protectedProcedure.query(({ ctx }) => { requireDriftRole(ctx.user, ["admin", "engineer", "user"]); return getOperationalWorldStateStatus(); }),
      finalAnalysis: protectedProcedure.input(z.object({ assetId: z.number().int().positive().optional() }).optional()).query(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer", "user"]); return getFinalIntelligence(input?.assetId); }),
      replay: protectedProcedure.input(z.object({ until: z.coerce.date().optional() }).optional()).query(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer"]); return replayOperationalEvents(input?.until); }),
      graphQuery: protectedProcedure.input(z.object({ kind: z.enum(["asset", "defect", "evidence", "mission", "telemetry", "event", "risk", "information_gap"]).optional(), severity: z.string().optional(), assetId: z.number().int().positive().optional(), relationship: z.enum(["affects", "observed_by", "supports", "targets", "captured_by", "propagates_to", "precedes", "contradicts"]).optional(), startId: z.string().optional(), hops: z.number().int().min(1).max(8).optional() })).query(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer", "user"]); return queryOperationalWorldGraph(input); }),
      graphAnalysis: protectedProcedure.input(z.object({ startId: z.string().optional(), hops: z.number().int().min(1).max(8).optional(), counterfactual: z.object({ nodeId: z.string(), event: z.enum(["failure", "repair", "inspection"]) }).optional() })).query(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer"]); return analyzeOperationalWorldGraph(input); }),
      simulateFailure: protectedProcedure.input(z.object({ assetIds: z.array(z.number().int().positive()).min(1).max(25) })).mutation(({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer"]); return simulateOperationalTwinFailure(input.assetIds); }),
    }),
  ...
  const evidenceRecord = await createEvidenceRecord({ missionId: input.missionId, uploadedBy: ctx.user?.id, fileName: input.fileName, mimeType: input.mimeType, storageKey: stored.key, storageUrl: browserStorageUrl(stored.key, stored.url), attachmentData: stored.attachmentData, mediaKind: input.mediaKind, latitude: input.latitude, longitude: input.longitude, playbackSeconds: input.playbackSeconds, source: input.captureSource, sha256: crypto.createHash("sha256").update(bytes).digest("hex"), capturedAt: input.capturedAt ? new Date(input.capturedAt) : undefined, cameraId: input.cameraId, captureZone: input.captureZone, headingDegrees: input.headingDegrees, qualityStatus: input.qualityStatus, imageQuality: input.imageQuality, provenance: { inspectionDomain: input.inspectionDomain, correlationKey: input.correlationKey, kind: input.captureSource === "hardware" ? "operator-uav-capture" : "operator-upload", aircraftProfile: input.aircraftProfile ?? null, originalCaptureRequired: true, notSimulator: true, uploadedWithoutSignIn: !ctx.user } });
        ...
        if (inference.detections?.length) await updateEvidenceDetections(evidenceRecord.id, inference.detections);
    ...
    reconstruction: router({
      ...
      create: publicProcedure.input(z.object({ name: z.string().min(3).max(220), fileName: z.string().min(1), mimeType: z.string(), sizeBytes: z.number().int().positive(), sourceUrl: z.string().url().refine(url => url.startsWith("https://"), "sourceUrl must use HTTPS so the worker can securely download the original capture."), latitude: z.number(), longitude: z.number(), altitudeMeters: z.number().positive(), cameraModel: z.string().optional(), flightMetadata: z.string().optional(), hasImu: z.boolean(), hasRtk: z.boolean(), hasBarometer: z.boolean(), hasIntrinsics: z.boolean(), durationSeconds: z.number().positive(), resolution: z.enum(["1080p", "4k"]) })).mutation(async ({ ctx, input }) => { const validation = validateCapture(input); if (!validation.valid) throw new Error(validation.errors.join(" ")); const plan = buildReconstructionPlan(input); const db = await getDb(); if (!db) throw new Error("DATABASE_URL is required to persist a reconstruction job."); const result = await db.insert(reconstructionJobs).values({ jobKey: plan.jobKey, name: input.name, status: "processing", inputFileName: input.fileName, inputMimeType: input.mimeType, inputSizeBytes: input.sizeBytes, latitude: String(input.latitude), longitude: String(input.longitude), altitudeMeters: Math.round(input.altitudeMeters), inputMetadata: input, qualityReport: plan.quality, stages: plan.stages, createdBy: ctx.user?.id ?? null }).returning({ id: reconstructionJobs.id, jobKey: reconstructionJobs.jobKey }); return { ...result[0], ...plan, message: "3D reconstruction started immediately from the uploaded capture." }; }),
