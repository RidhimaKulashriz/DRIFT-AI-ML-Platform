...
export const appRouter = router({
  ...
  drift: router({
    ...
    reconstruction: router({
      ...
      create: protectedProcedure.input(z.object({ name: z.string().min(3).max(220), fileName: z.string().min(1), mimeType: z.string(), sizeBytes: z.number().int().positive(), latitude: z.number(), longitude: z.number(), altitudeMeters: z.number().positive(), cameraModel: z.string().optional(), hasImu: z.boolean(), hasRtk: z.boolean(), hasBarometer: z.boolean(), hasIntrinsics: z.boolean(), durationSeconds: z.number().positive(), resolution: z.enum(["1080p", "4k"]) })).mutation(async ({ ctx, input }) => { requireDriftRole(ctx.user, ["admin", "engineer", "user"]); const validation = validateCapture(input); if (!validation.valid) throw new Error(validation.errors.join(" ")); const plan = buildReconstructionPlan(input); const db = await getDb(); if (!db) throw new Error("DATABASE_URL is required to persist a reconstruction job."); const result = await db.insert(reconstructionJobs).values({ jobKey: plan.jobKey, name: input.name, status: "queued", inputFileName: input.fileName, inputMimeType: input.mimeType, inputSizeBytes: input.sizeBytes, latitude: String(input.latitude), longitude: String(input.longitude), altitudeMeters: Math.round(input.altitudeMeters), inputMetadata: input, qualityReport: plan.quality, stages: plan.stages, createdBy: ctx.user.id }).returning({ id: reconstructionJobs.id, jobKey: reconstructionJobs.jobKey }); return { ...result[0], ...plan, message: "Capture accepted. Reconstruction pipeline is queued; no geometry is fabricated until processing completes." }; }),
