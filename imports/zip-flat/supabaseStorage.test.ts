...
describe("Supabase portable evidence storage activation", () => {
  it("recognizes only well-formed Supabase-backed storage references", () => {
    ...
    expect(isSupabaseStorageKey("https://storage.example/frame.jpg")).toBe(false);
  });

  it("remains fail-closed until an explicit server-side enablement flag is set", () => {
