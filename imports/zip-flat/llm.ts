...
const resolveApiUrl = () => {
  if (!ENV.forgeApiUrl || ENV.forgeApiUrl.trim().length === 0) throw new Error("Configured LLM provider URL is missing");
  return `${ENV.forgeApiUrl.replace(/\/$/, "")}/v1/chat/completions`;
};

const assertApiKey = () => {
  if (!ENV.forgeApiKey) {
    throw new Error("Configured LLM provider key is missing");
  }
};
