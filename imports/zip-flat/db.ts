...
export async function assignContractorUserRecord(input: { contractorId: number; userId: number; assignedBy: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable. Configure PostgreSQL before assigning a contractor user.");
...
export async function registerAuthorizedSecurityObservationRecord(input: { assetId?: number; cameraSourceId?: number; source: "authorized_bridge_health" | "approved_security_adapter"; integrationName: string; sourceRecordReference: string; observationType: "bridge_health_signal" | "security_adapter_alert"; observationSummary: string; authorizedScope: string; retentionUntil: Date; observedAt: Date; integrityMetadata: Record<string, unknown>; createdBy: number }) {
  ...
  if (input.assetId) {
    const asset = (await db.select({ id: assets.id }).from(assets).where(eq(assets.id, input.assetId)).limit(1))[0];
    if (!asset) throw new Error("The referenced project asset does not exist.");
  }
  if (input.cameraSourceId) {
    const camera = (await db.select({ id: cameraSources.id, retentionUntil: cameraSources.retentionUntil }).from(cameraSources).where(eq(cameraSources.id, input.cameraSourceId)).limit(1))[0];
    if (!camera) throw new Error("The referenced authorized camera source does not exist.");
    if (camera.retentionUntil <= new Date()) throw new Error("The referenced camera authorization has expired.");
  }
...
export async function prepareUavFollowUpRecommendationRecord(input: { ticketId: number; cctvCandidateId?: number; triggerReason: string; expiresAt: Date; preparedBy: number }) {
  ...
  const requiredChecks = {
    preparedOnly: true,
    noAircraftCommand: true,
    operatorIdentityRequired: true,
    assetOwnerAuthorizationRequired: true,
    airspaceAndJurisdictionCheckRequired: true,
    weatherAndPreflightCheckRequired: true,
    originalFollowUpMediaRequired: true,
  };
