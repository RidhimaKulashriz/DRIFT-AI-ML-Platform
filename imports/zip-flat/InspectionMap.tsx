...
export function InspectionMap({ defects, telemetry, selectedId, streetViewRequest = 0, onSelect, className }: InspectionMapProps) {
  ...
  useEffect(() => {
    ...
    CAMPUS_SITES.forEach(site => {
      const marker = new window.google.maps.Marker({ map, position: { lat: site.latitude, lng: site.longitude }, title: `Campus reference: ${site.name}`, label: { text: site.shortName === "IIIT DELHI" ? "IIIT" : "IGD", color: "#ffffff", fontSize: "9px", fontWeight: "700" }, icon: { path: window.google.maps.SymbolPath.BACKWARD_CLOSED_ARROW, fillColor: "#047481", fillOpacity: .98, strokeColor: "#ffffff", strokeWeight: 1.5, scale: 7 } });
      marker.addListener("click", () => {
        infoWindow.setContent(`<div style="max-width:260px;font:13px Arial,sans-serif"><strong>Verified campus reference only</strong><br/>${site.name}<br/>${site.address}<br/>Coordinates: ${site.latitude.toFixed(6)}, ${site.longitude.toFixed(6)}<br/><em>Not a DRIFT finding, inspection record, evidence location, or safety determination.</em></div>`);
        infoWindow.open({ map, anchor: marker });
      });
      contextMarkers.current.push(marker);
    });
  ...
  const focusCampus = (siteId: (typeof CAMPUS_SITES)[number]["id"]) => {
    const map = mapRef.current;
    const site = CAMPUS_SITES.find(candidate => candidate.id === siteId);
    if (!map || !site) return;
    map.getStreetView().setVisible(false);
    setStreetViewStatus("idle");
