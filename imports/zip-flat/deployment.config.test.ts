...
describe("external deployment artifacts", () => {
  ...
  it("keeps the Vercel map surface independent of the Manus Forge proxy and serves a favicon", () => {
    ...
    expect(driftMap).toContain("maps.googleapis.com/maps/api/js");
    expect(driftMap).toContain("Google Maps infrastructure context");
    expect(driftMap).toContain("Public NBI context only");
    expect(driftMap).toContain("Not a DRIFT site, live defect, ticket, or safety determination");
  ...
  it("keeps Street View and numbered temporary markers available without treating public imagery as DRIFT evidence", () => {
    ...
    const driftMap = readFileSync(resolve(root, "client/src/components/DriftMap.tsx"), "utf8");
    const consoleSource = readFileSync(resolve(root, "client/src/pages/DriftConsole.tsx"), "utf8");
    expect(driftMap).toContain("streetViewControl: true");
    expect(driftMap).toContain("OPEN STREET VIEW");
    expect(driftMap).toContain("Street View is not available within 250 m");
    expect(driftMap).toContain("not DRIFT evidence or a defect confirmation");
    expect(driftMap).toContain("SIMULATED DEMO advisory");
    expect(driftMap).toContain("numbered temporary advisory");
  ...
  it("gives every temporary report advisory its own map and Street View inspection action", () => {
    ...
    expect(driftMap).toContain("streetViewRequest?: number");
    expect(driftMap).toContain("completedStreetViewRequest");
    expect(driftMap).toContain("select any marker or report item");
  });

  it("keeps the temporary advisory grid readable by making telemetry optional and focusing the simulated grid", () => {
    const root = resolve(import.meta.dirname, "..");
    const driftMap = readFileSync(resolve(root, "client/src/components/DriftMap.tsx"), "utf8");
    expect(driftMap).toContain("const [telemetryVisible, setTelemetryVisible] = useState(false)");
    expect(driftMap).toContain("const shouldShowTelemetry = telemetryVisible || validDefects.length === 0");
    expect(driftMap).toContain("FOCUS 15-POINT GRID");
    expect(driftMap).toContain("SHOW ${validTelemetry.length} TELEMETRY");
    expect(driftMap).toContain("map.panTo(selectedDefect.point)");
    expect(driftMap).toContain("map.fitBounds(bounds, validDefects.length ? 54 : 84)");
  });
