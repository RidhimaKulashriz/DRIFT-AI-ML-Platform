...
- [x] Add a public transient simulator path for no-persistence environments; it displays only browser-session advisory telemetry and map coordinates, explicitly stores nothing, and cannot create project missions, evidence, reports, contractor tickets, CCTV candidates, security observations, or UAV actions.
- [x] Make transient simulator map markers resolve only to a matching transient advisory detail state with no persistent mission, asset, evidence, contractor, CCTV, security, report, or UAV linkage.
- [x] Add a browser-only transient demo briefing preview and Markdown download that uses only current simulator output, explicitly excludes engineering use and persistent records, and cannot create a real report, ticket, CCTV candidate, security observation, or UAV action.
...
- [x] Implement and verify contractor-specific data scoping before granting contractor users access to assigned real work records.
