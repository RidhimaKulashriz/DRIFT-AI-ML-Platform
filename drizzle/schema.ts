...
export const securityObservations = pgTable("securityObservations", { id: serial("id").primaryKey(), assetId: integer("assetId"), cameraSourceId: integer("cameraSourceId"), source: securityObservationSourceEnum("source").notNull(), integrationName: varchar("integrationName", { length: 160 }).notNull(), sourceRecordReference: varchar("sourceRecordReference", { length: 240 }).notNull().unique(), observationType: varchar("observationType", { length: 120 }).notNull(), observationSummary: text("observationSummary").notNull(), authorizedScope: text("authorizedScope").notNull(), retentionUntil: timestamp("retentionUntil", { withTimezone: true }).notNull(), observedAt: timestamp("observedAt", { withTimezone: true }).notNull(), integrityMetadata: jsonb("integrityMetadata").notNull(), status: securityObservationStatusEnum("status").notNull().default("pending_review"), reviewedBy: integer("reviewedBy"), reviewedAt: timestamp("reviewedAt", { withTimezone: true }), createdBy: integer("createdBy").notNull(), createdAt: createdAt(), updatedAt: updatedAt() });
